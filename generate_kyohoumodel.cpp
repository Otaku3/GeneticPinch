...
int main(){
  ...
  ostr_ply << "kyohoumodel_" << result <<".ply";
  ostr_csv << "kyohoumodel_" << result <<".csv";
  ...
  //花軸を生成(z軸上青色，vertex1000)>>巨峰は15段なので[-7.5:7.5]で花軸を置く
  int count_vertex = 0;
  ...
  for(int i=1; i<=4; i+=1){
    //sample重心座標の各成分を生成(花軸の範囲内)
    double center_x = rand_center4(mt);
    double center_y = rand_center4(mt);
    double center_z = rand_z(mt);
